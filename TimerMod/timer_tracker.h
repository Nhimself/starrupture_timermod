...
namespace RuptureTimer
{
	...
	// Called by every TimerMod instance at init to generate this instance's broadcast identity.
	// The instance with the lowest serverId (started earliest) wins the broadcaster election.
	void InitServerMode();

	// Returns the serverId generated by InitServerMode() for this instance.
	uint32_t GetServerId();

	// Called from the Network::OnReceive handler in plugin.cpp when a broadcast arrives.
	// Implements the election: lower serverId wins; this instance defers to it and stops
	// broadcasting. Packets from higher-ID instances are discarded.
	void ApplyNetworkSync(const TimerSyncPacket& pkt);

	// Returns true if this instance has deferred to an earlier-started broadcaster.
	// Used by OnEngineTick to suppress outgoing broadcasts once we are a pure receiver.
